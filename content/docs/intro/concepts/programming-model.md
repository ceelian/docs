...
There are three kinds of `Asset`:
...
Note that a folder may be passed to `FileArchive` to constuct an archive from the contents of that folder.  Note also that both assets (single files) and archives (folders containing files) can be combined as part of building up an `AssetArchive`.
